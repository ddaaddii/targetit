        ...
        <div class="card-body">
            ...
            <form method="POST" action="/enderecos/{{ $endereco->id }}">
                    ...
                    <div class="col-md-6">
                        <select  class="form-control"  name="user_id" id="user_id"  data-placeholder="Selecione o tipo de usuario">
                            @foreach ($usuarios as $tipo)
                                <option {{ old('user_id', $endereco->user_id) == $tipo->id ? 'selected' : '' }} value="{{ $tipo->id }}">{{ $tipo->name }}</option>
                            @endforeach
                         </select>
                        @error('user_id')
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <input id="cep" type="text" class="form-control " name="cep" value="{{ old('cep', $endereco->cep) }}" required>
                    </div>
                    @error('cep')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
